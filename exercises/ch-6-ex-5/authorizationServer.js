...
app.post("/approve", function (req, res) {
  ...
  if (req.body.approve) {
    if (query.response_type == "code") {
      ...
      var token_response = generateTokens(
        req,
        res,
        query.clientId,
        user,
    ...
    if (code) {
      ...
      if (code.authorizationEndpointRequest.client_id == clientId) {
        ...
        if (!user) {
          ...
          res
            .status(500)
            .render("error", { error: "Unknown user " + code.user });
          return;
        }
        console.log("User %j", user);
        ...
        var token_response = generateTokens(
          ...
          user,
          code.scope,
          code.authorizationEndpointRequest.nonce,
          true,
        );
...
app.post("/introspect", function (req, res) {
  var auth = req.headers["authorization"];
  var resourceCredentials = Buffer.from(auth.slice("basic ".length), "base64")
    .toString()
    .split(":");
...
app.get(
  "/register/:clientId",
  validateConfigurationEndpointRequest,
  function (req, res) {
